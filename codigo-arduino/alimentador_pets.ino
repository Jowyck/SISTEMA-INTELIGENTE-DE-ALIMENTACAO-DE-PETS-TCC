...
const int botao_racao = 12;      // Botão para liberar ração (modo sensor) - PULL-DOWN
const int buzzer = 2;            // Buzzer Ativo WJ
const int led = 13;              // LED Difuso Genérico

// Variáveis globais
int modo = 0;                    // 0=não selecionado, 1=Sensor, 2=Timer
...
int opcaoSelecionada = 1;        // 1=Sensor, 2=Timer
...
int estadoBluetooth = 0; // 0=Aguardando conexão, 1=Conectado, 2=Ajuste RTC, 3=Seleção Modo, 4=Sensor, 5=Timer
...
void processarBluetooth() {
  ...
  if (stringComplete) {
    ...
    // CORREÇÃO CRÍTICA: VOLTMODOS deve ir para Seleção de Modo (estado 3), não para Ajuste RTC (estado 2)
      ...
      modoConfigurado = true;
      bluetoothConnected = true;
      estadoBluetooth = 4; // Modo Sensor ativo
      forcarExibicaoRacao = false;
      lcd.clear();
      lcd.print("BT:Sensor");
      enviarEstadoAgua();
...
void selecionarModo() {
  ...
  lcd.print("Selecione:       ");
  lcd.setCursor(0, 1);
  if (opcaoSelecionada == 1) lcd.print(">Modo 1 Sensor   ");
...
void exibirModoAtivo(int modoAtivo, DateTime now) {
  ...
  lcd.print("Modo ");
  lcd.print(modoAtivo);
  lcd.print(modoAtivo == 1 ? " Sensor" : " Timer");
